...
$output = foreach($group in $groupNames)
{
 ...
 foreach($member in $members)
 {
    ...
    Get-ADUser $member -properties * | Where-Object {$_.DepartmentNumber -like "*0641*"}| Select-Object SamAccountName, EmployeeID, DisplayName, Title, Enabled, Department, UserPrincipalName,  @{Name = 'License'; Expression = {$group}}
